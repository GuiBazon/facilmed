...
/**
 * Ferramentas de Tool Calling da MedIA conforme a arquitetura oficial do projeto
 */
...
/**
 * Processador Inteligente Completo da MedIA
 */
async function processarChatMedIA(mensagemUsuario, historico = [], pacienteId = 1) {
  ...
  // A. Informações da Clínica / Endereço / Telefone
  if (texto.includes('onde') || texto.includes('endereço') || texto.includes('endereco') || texto.includes('telefone') || texto.includes('contato') || texto.includes('local') || texto.includes('funciona')) {
    return {
      resposta: `🏥 **FácilMed Clínica Multiprofissional**\n\n📍 **Endereço:** Av. Paulista, 1000 — São Paulo / SP\n📞 **Telefone:** (11) 4004-9988 / WhatsApp: (11) 98877-6655\n🕒 **Horário de Atendimento:** Segunda a Sexta das 08h às 17h | Sábados das 08h às 12h.`
    };
  }

  // B. Listar Médicos / Especialidades disponíveis
  if (texto.includes('quais médicos') || texto.includes('quais medicos') || texto.includes('quais especialidades') || texto.includes('doutores') || texto.includes('quem atende') || texto.includes('profissionais')) {
    const medicosList = await query(`SELECT m.nome, m.crm, m.valor_consulta, e.nome as especialidade FROM medicos m JOIN especialidades e ON m.especialidade_id = e.id`);
    const linhas = medicosList.map(m => `• **${m.nome}** (${m.especialidade}) — ${m.crm} — R$ ${m.valor_consulta.toFixed(2)}`).join('\n');
    return {
      resposta: `👨‍⚕️ **Nossos Médicos e Especialidades:**\n\n${linhas}\n\nQuer agendar com algum deles? É só me falar!`
    };
  }

  // C. Dúvidas sobre Convênio / Pagamento
  if (texto.includes('convenio') || texto.includes('convênio') || texto.includes('bradesco') || texto.includes('amil') || texto.includes('sulamerica') || texto.includes('unimed') || texto.includes('aceita')) {
    return {
      resposta: `💳 **Aceitamos Convênio e Particular!**\n\nAtendemos os principais convênios (Bradesco Saúde, Amil, SulAmérica, Unimed, etc.).\nNo agendamento pelo app ou aqui pelo chat, basta nos informar o código da sua carteirinha!`
    };
  }

  // D. Responsável / Acompanhante
  if (texto.includes('responsavel') || texto.includes('responsável') || texto.includes('acompanhante') || texto.includes('cuidador') || texto.includes('dependente')) {
    const res = await executarToolCall('buscar_responsavel', { paciente_id: pacienteId }, pacienteId);
    if (res.responsaveis && res.responsaveis.length > 0) {
      const r = res.responsaveis[0];
      return {
        resposta: `👥 **Responsável Vinculado:**\n\n• **Nome:** ${r.responsavel_nome}\n• **Telefone:** ${r.responsavel_telefone}\n• **Vínculo:** ${r.parentesco}\n\nO responsável tem autorização para visualizar consultas, confirmar presença e reagendar em seu nome!`,
        toolExecuted: 'buscar_responsavel',
        toolResult: res
      };
    } else {
      return { resposta: `Você ainda não possui um responsável/acompanhante cadastrado. No app você pode autorizar um familiar!` };
    }
  }

  // E. Orientações ("o que levar", "preparo", "instruções")
  ...
  // F. Reagendamento ("remarcar", "reagendar", "mudar data")
  ...
  if (texto.includes('confirmar') || texto.includes('vou ir') || texto.includes('presença') || texto.includes('presenca')) {
    const agendamento = await get(`SELECT * FROM agendamentos WHERE paciente_id = ? AND status = 'AGENDADO' ORDER BY id DESC LIMIT 1`, [pacienteId]);
  ...
  // H. Buscar / Minha Próxima Consulta
  if (texto.includes('minha') || texto.includes('proxima') || texto.includes('próxima') || texto.includes('quais consultas') || texto.includes('tenho consulta')) {
  ...
  // L. Saudação Geral & Guia Completo de Comandos
  return {
    resposta: `Olá! Eu sou a **MedIA**, a Secretária Virtual do FácilMed 👩‍⚕️.\n\nPosso te ajudar com qualquer uma das opções abaixo:\n\n• *"Quais os médicos que atendem aqui?"*\n• *"Ver horários com a Dra. Ana"*\n• *"Agendar consulta com Dr. Roberto amanhã às 09:00"*\n• *"O que levar para a consulta de Ortopedia?"*\n• *"Confirmar minha presença"*\n• *"Quem é meu responsável cadastrado?"*\n• *"Qual o endereço e telefone da clínica?"*`
  };
}
